#pragma once

const static std::string kSahderVertexLighting		= "kSahderVertexLighting";
const static std::string kShaderFragmentLighting	= "kShaderFragmentLighting";
const static std::string kShaderScreenSpace			= "kShaderScreenSpace";
const static std::string kShaderUnlit				= "kShaderUnlit";
const static std::string kShaderDebug				= "kShaderDebug";
const static std::string kShaderScreenSpaceBW		= "kShaderScreenSpaceBW";
const static std::string kSahderClouds				= "kSahderClouds";
const static std::string kShaderShip				= "kShaderShip";
const static std::string kShaderScreenText			= "kShaderScreenText";

const static float kToRad							= 0.017453292519943295769f;
const static float kToDeg							= 0.017453292519943295769;